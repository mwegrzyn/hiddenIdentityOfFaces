...
expName ='famFaceFilter'  # from the Builder filename that created this script
...
imageBekannt = visual.ImageStim(win=win, name='imageBekannt',
    image='sin', mask=None,
    ori=0, pos=[0, 120], size=[300, 300],  #############
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
ratingBekannt = visual.RatingScale(win=win, name='ratingBekannt', marker=u'hover', size=0.8, pos=[0.0, -200.0], choices=[u'1.ja', u'2.nein'], tickHeight=-1, singleClick=True, showAccept=False)
...
# three lists, each time the same pics but filtered differently
# for each list, there psychopy does a random shuffle, so that
# each list is presented in a different order
condLists = [u'smooList.csv',u'edgeList.csv',u'normList.csv']

for condCsv in condLists:

    # set up handler to look after randomisation of conditions etc
    trials = data.TrialHandler(nReps=1, method='random',
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(condCsv),
